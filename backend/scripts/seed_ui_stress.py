...
ALERTNAMES = [
    "DemoHighCPUUsage", "DemoHighMemoryUsage", "DemoDiskWillFillIn4Hours",
    "DemoHostOutOfMemory", "DemoHighDiskIO", "DemoPacketLoss", "DemoTargetDown",
    "DemoHighLatencyP99", "DemoTooManyConnections", "DemoCertExpiringSoon",
]
...
async def clear(db) -> dict[str, int]:
    ...
    for label, stmt in [
        ...
        ("incident_events", delete(IncidentEvent).where(IncidentEvent.incident_id.in_(demo_incidents))),
        ("incidents", delete(Incident).where(Incident.group_key.like(f"{GK}%"))),
        ("mutes", delete(NotificationMute).where(
        ...
        ("threshold_overrides", delete(ThresholdOverride).where(
...
async def seed(db) -> dict[str, int]:
    ...
    for i in range(1, 51):
        ...
        s = Server(name=host, cmdb_ci=cmdb, labels=labels,
                   description=f"{tier} node {i}", server_group_id=grp.id)
        db.add(s)
        servers.append(s)
    await db.flush()
    ...
    def new_incident(server, alertname, severity, status, n, hours_ago):
        ...
        inc = Incident(
            title=f"{alertname} on {server.name}", status=status, severity=severity,
            group_key=f"host={server.name}", first_seen=first,
            last_seen=first + timedelta(minutes=6 * max(n, 1)), alert_count=n,
            notified_at=now,
        )
        db.add(inc)
    ...
    def alert(inc, server, alertname, severity, j, first, *, labels=None):
        ...
        ev = AlertEvent(
            fingerprint=f"{FP}{inc.group_key}-{alertname}-{j}", source="alertmanager",
            name=alertname, severity=severity, status="firing",
            labels=labels or {"host": server.name, "cmdb_ci": server.cmdb_ci,
                              "instance": f"{ip}:9100", "cmdb_hostname": server.name, "job": "node"},
            ...
            incident_id=inc.id, dedup_count=j + 1,
        )
        db.add(ev)
        return ev
    ...
    for j in range(20):
        ...
        await db.flush()
        db.add(IncidentEvent(incident_id=inc.id, kind="alert_attached",
                             payload={"alert_event_id": str(ev.id), "name": ev.name}))

    # 2) MANY-LABEL incident (one alert, ~18 labels incl. long values)
    s = servers[2]
    inc, first = new_incident(s, "DemoTooManyConnections", "warning",
                              IncidentStatus.acknowledged, 1, 4)
    await db.flush()
    db.add(IncidentEvent(incident_id=inc.id, kind="created", payload={}))
    big = {
        "host": s.name, "cmdb_ci": s.cmdb_ci, "cmdb_hostname": s.name,
        "instance": f"{s.labels['cmdb_ip']}:9187", "cmdb_env": "prod",
        "cmdb_owner": "team-payments", "cmdb_service_l1": "ledger", "cmdb_location": "icn-dc1",
        "job": "postgres-exporter", "datname": "payments_prod", "namespace": "payments",
        "pod": "pg-primary-0", "container": "postgres", "severity": "warning",
        "team": "team-payments",
    ...
    ev.annotations = {"summary": "connections 480/500",
    ...
    s = servers[5]
    inc, first = new_incident(s, "DemoDiskWillFillIn4Hours", "critical",
                              IncidentStatus.suppressed, 3, 3)
    await db.flush()
    for kind, payload in [
        ("created", {}),
        ("alert_attached", {"alert_event_id": "demo", "name": "DemoDiskWillFillIn4Hours"}),
        ("status_changed", {"from": "open", "to": "acknowledged", "actor": "oncall-kim",
                            "note": "investigating, looks like log rotation stalled"}),
        ("comment", {"author": "oncall-lee", "text": "Cleared 40GB of old WALs; monitoring. "
                     "Root cause likely the archiver falling behind after the 2am deploy.",
                     "attachments": [{"type": "grafana", "url": "https://g/d/abc"}]}),
        ("notification_muted", {"reason": "planned maintenance window",
                                "muted_pairs": [{"cmdb_ci": s.cmdb_ci,
                                                 "alertname": "DemoDiskWillFillIn4Hours"}]}),
        ("status_changed", {"from": "acknowledged", "to": "suppressed", "actor": "oncall-kim"}),
        ("llm_analysis", {"root_cause": "WAL archiver backlog", "confidence": 0.82,
                          "recommended_actions": ["scale archiver", "raise disk alert floor"],
                          "model": "gpt-4o-mini", "tokens": {"prompt": 1820, "completion": 240}}),
    ]:
    ...
    statuses = [IncidentStatus.open, IncidentStatus.acknowledged,
                IncidentStatus.resolved, IncidentStatus.suppressed]
    ...
    db.add(ThresholdOverride(alertname="DemoHighCPUUsage", tier="server",
                             target_cmdb_ci=servers[1].cmdb_ci, value=95.0))
    db.add(ThresholdOverride(alertname="DemoHighCPUUsage", tier="group",
                             target_group_id=groups[0].id, value=90.0))
    db.add(ThresholdOverride(alertname="DemoHighMemoryUsage", tier="group",
                             target_group_id=groups[2].id, value=85.0))
    db.add(ThresholdOverride(alertname="DemoHostOutOfMemory", tier="server",
                             target_cmdb_ci=servers[2].cmdb_ci, value=512.0))
    db.add(ThresholdOverride(alertname="DemoDiskWillFillIn4Hours", tier="group",
                             target_group_id=groups[4].id, value=88.0))
    db.add(ThresholdOverride(alertname="DemoHighLatencyP99", tier="server",
                             target_cmdb_ci=servers[0].cmdb_ci, value=250.0))
    db.add(ThresholdOverride(alertname="DemoHighDiskIO", tier="group",
                             target_group_id=groups[3].id, value=5000.0))

    # notification mutes
    db.add(NotificationMute(target_type="server", target_cmdb_ci=servers[5].cmdb_ci,
                            alertname="DemoDiskWillFillIn4Hours", reason="planned maintenance"))
    db.add(NotificationMute(target_type="group", target_group_id=groups[4].id,
                            alertname=None, reason="batch tier muted overnight"))
    db.add(NotificationMute(target_type="server", target_cmdb_ci=servers[9].cmdb_ci,
                            alertname=None, reason="decommissioning"))
    db.add(NotificationMute(target_type="all", target_cmdb_ci=None,
                            alertname="DemoCertExpiringSoon", reason="cert renewal in progress"))

    await db.commit()
    return {"groups": len(GROUPS), "servers": 50, "incidents": len(incidents),
            "catalog": len(catalog), "thresholds": 7, "mutes": 4}
...
async def main() -> None:
    ...
    async with async_session_factory() as db:
        ...
        print("SEED_OK", created,
              "(incl. 1x20-alert, 1x18-label, 1x rich-JSON-timeline)")
